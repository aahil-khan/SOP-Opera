...
**Retrieved references** (regulations, historical incidents, SOPs) are inputs to the AI retrieval stage — fetched via hybrid retrieval (RAG or deterministic fallback) under Orchestrator control — not Evidence until frozen at Decision time.
...
One process, one database, one frontend app. The Simulator runs as a background asyncio task inside the same FastAPI process — it is not a separate service, and it talks to the Context Engine through the exact same internal interface Manual Input uses. PostgreSQL hosts **pgvector** as an extension (not a separate vector service) so semantic retrieval lives in the same database — preserving the "no infrastructure the demo doesn't need" principle.
...
5. The **Assessment Orchestrator** (Section 5.4) takes the job: it deterministically decides, from the active Derived Facts alone, whether retrieval is required and builds the retrieval query. Retrieval is **hybrid** (Section 5.4.3): RAG over `knowledge_chunks` via pgvector is attempted first; a quality gate grades the result; weak/empty/timeout falls back to deterministic SQL `RETRIEVAL_RULES`. It then builds a prompt from Derived Facts + Context references + any retrieved references, calls the active AI provider, validates the structured response, retries once on failure, and persists an AI Assessment + Recommendations (or marks the job `failed`). On failure, the supervisor may instead submit a Manual Assessment (Section 5.4.2).
...
DERIVED_FACT_RULES: list[Callable[[list[ContextRecord]], DerivedFact | None]] = [
    # Canonical trio — required by Compound Risk (Section 12); map to PS examples
    ...
    # Beyond the problem-statement examples — product thinking, not PS karaoke
    rule_incomplete_isolation,    # LOTO / isolation not confirmed for the work window → Incomplete Isolation
    rule_simultaneous_ops,        # Adjacent hot-work + confined-space / SIMOPS conflict → Simultaneous Ops
    rule_certification_expiring,  # Assigned worker cert missing or inside expiry window → Certification Expiring
]
...
class AssessmentOrchestrator:
    ...
    Responsibilities:
      ...
      - decide, deterministically, whether retrieval is required and build the query
      - run hybrid retrieval (RAG primary → quality gate → deterministic fallback)
      - build the prompt from Derived Facts + Context + retrieved references
      ...
      - write the AI Observability record on every attempt (incl. retrieval_mode)
...
**Deterministic Orchestration, Hybrid Retrieval.** The Orchestrator—not the LLM—decides whether additional references are required and constructs the retrieval query from active Derived Facts, review type, and asset category. There is **no autonomous tool calling or agentic behavior**; the LLM only reasons over the information it is given. What *can* vary is the retrieval *mechanism*: semantic RAG first, deterministic SQL as a guaranteed floor (Section 5.4.3). This supersedes archived ADR-006 ("Deterministic Retrieval only").

```python
# Fallback map — used when RAG quality gate fails. Always kept; never deleted.
...
class RetrievedReference(BaseModel):
    ...
    id: UUID                          # row in regulations / incidents / sops
    retrieval_path: Literal["rag", "deterministic"]
    score: float | None               # cosine/similarity when path=rag; None otherwise
    chunk_id: UUID | None             # knowledge_chunks.id when path=rag
...
Flow: retrieve (if required, Section 5.4.3) → generate → validate → **on failure, one retry with a repair instruction appended** → on second failure, `status = failed`, Review stays in `Assessing`, visibly. Recommendations are a field on the Assessment result, not a separate pipeline stage.

#### 5.4.3 Hybrid Retrieval

Two-tier retrieval under Orchestrator control. Deterministic SQL lookup is **never deleted** — it is the guaranteed floor so RAG can degrade without breaking the Assessment loop.
...
1. Orchestrator builds a natural-language query from active Derived Facts + review type + asset category, and decides required `source_types` (same categories as `RETRIEVAL_RULES`).
2. `RagRetriever` embeds the query (OpenAI-compatible embeddings primary; local embedding fallback) and runs pgvector cosine search over `knowledge_chunks`, hard timeout ~3s.
3. `assess_retrieval_quality()` grades the result: **good** (≥ min chunks with score ≥ threshold), **weak** (some hits below bar), **empty** (zero usable hits / timeout / embed error).
4. On **good** → use RAG refs (`retrieval_path = "rag"`). On **weak** / **empty** / timeout / error → call `DeterministicRetriever` (`retrieval_path = "deterministic"`).
5. Both paths emit `RetrievedReference` objects of the same shape. The Digital Twin "Retrieved References" node and AI Ops show path + score so judges can see that retrieval is real, not a slide claim.

Config (`.env` / Pydantic Settings): `RAG_ENABLED`, `EMBEDDING_MODEL`, `EMBEDDING_DIM`, `RAG_TOP_K`, `RAG_SCORE_THRESHOLD`, `RAG_TIMEOUT_MS`, `EMBEDDING_PROVIDER` (`openai_compatible` | `local` | `mock`).

Corpus is seeded at boot via `db/seed_embeddings.py`: regulations, historical incidents, and SOPs are chunked and pre-embedded so replay stays deterministic for demos.
...
class AssessmentMetadata(BaseModel):
    ...
    retrieved_evidence_ids: list[UUID]   # RetrievedReference.source ids cited
    retrieval_mode: Literal["rag", "deterministic", "skipped"]
    retrieval_quality: Literal["good", "weak", "empty", "n_a"]
    retrieval_score: float | None        # best RAG score when retrieval_mode=rag
...
The AI Ops dashboard (Section 6) is a read-only aggregation view over this table. In addition to standard aggregations, the dashboard tracks **Assessment Success Rate**, **Failed Assessments**, **Validation Failures**, **RAG Hit Rate**, **RAG Fallback Rate**, and **Mean Retrieval Relevance** to provide immediate visibility into pipeline health and model reliability during the demo.
...
**Reasoning trace panel.** The side panel lays out, top to bottom, exactly why the Assessment reached its conclusion: Asset → Context → Derived Facts → Retrieved References → Assessment → Recommendations → Decision. The **Retrieved References** node shows retrieval path (`rag` / `deterministic`) and relevance score when available. Before a Decision is submitted this shows live Context; after Decision, the cited snapshot is Evidence.
...
**Stack:** Next.js (App Router) + TypeScript + React. TanStack Query for server state. Minimal global client state (Zustand). Live surfaces (Digital Twin, WebSocket client, Demo Mode) are **Client Components**; the app still talks to **FastAPI** for all REST and WebSocket — do not put business/Assessment logic in Next route handlers.

**Why Next (vs Vite SPA):** team familiarity and simpler deploy/hosting for the UI shell. Architecture is unchanged: Next is the presentation app; FastAPI remains the only API and real-time backend.

**Structure:**

- App opens on Digital Twin in Demo Mode (`app/page.tsx` or equivalent).
...
- `AIOpsDashboard` — read-only, polls/subscribes to aggregate metrics including Assessment Success Rate, Failed Assessments, Validation Failures, RAG Hit Rate, RAG Fallback Rate, and Mean Retrieval Relevance.
...
class DerivedFact(BaseModel):
    id: UUID
    asset_id: UUID
    # elevated_gas | permit_conflict | zone_occupied |
    # incomplete_isolation | simultaneous_ops | certification_expiring
    fact_type: str
...
PostgreSQL, one schema. Audit is insert-only by team convention. Enable `**CREATE EXTENSION IF NOT EXISTS vector;`** (pgvector) so `knowledge_chunks.embedding` lives in the same database — no separate vector service.


| Table                 | Key columns                                                                                                                                                                                          | Notes                                                                                 |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `assets`              | id, name, zone, plant_id                                                                                                                                                                             | Seeded fixture                                                                        |
| `workers`             | id, name, certifications, department_id                                                                                                                                                              | Seeded fixture; certifications JSON includes expiry for `rule_certification_expiring` |
| `departments`         | id, name                                                                                                                                                                                             | Seeded fixture                                                                        |
| `permits`             | id, asset_id, worker_ids, window_start, window_end, status, work_type                                                                                                                                | Seeded fixture + simulator-writable; `work_type` supports SIMOPS / isolation rules    |
| `review_types`        | id, name                                                                                                                                                                                             | Seeded fixture                                                                        |
| `incidents`           | id, asset_id, description, reported_at, linked_review_ids                                                                                                                                            | Used for post-closure linking & Retrieval / chunking source                           |
...
| `knowledge_chunks`    | id, source_type, source_id, chunk_text, embedding (vector), applies_to_category, token_count                                                                                                         | Chunked + pre-embedded at seed; RAG corpus                                            |
...
| `context_entries`     | id, asset_id, category, payload (jsonb), provider, valid_from, valid_until, confidence                                                                                                               | Categories include isolation status, adjacent ops, etc.                               |
| `derived_facts`       | id, asset_id, fact_type, value, computed_at, source_context_ids                                                                                                                                      | `fact_type` includes the six MVP rules                                                |
| `assessments`         | id, review_id, assessment_type, status, risk_level, summary, derived_fact_ids, version                                                                                                               | `assessment_type`: `ai` | `manual`                                                    |
| `assessment_metadata` | assessment_id, provider, model, prompt_version, tokens_in, tokens_out, cost_usd, latency_ms, confidence, retrieved_evidence_ids, retrieval_mode, retrieval_quality, retrieval_score, embedding_model | RAG observability columns included                                                    |
...
backend/
  app/
    ...
    assessment/        orchestrator.py, pipeline.py, manual.py, schemas.py,
                       retrieval/
                         __init__.py          # facade: hybrid retrieve()
                         rag.py               # RagRetriever + quality gate
                         deterministic.py     # RETRIEVAL_RULES SQL path
                       embeddings/
                         openai_compatible.py
                         local.py
                         mock.py
                       providers/openai_compatible.py,
                       providers/ollama.py, providers/mock.py
    ...
    db/                schema.sql, session.py, seed.py, seed_embeddings.py
    main.py

frontend/                 # Next.js App Router
  app/
    layout.tsx
    page.tsx              # opens on Digital Twin in Demo Mode
    reviews/
    assessments/
    reports/
    ai-ops/
...
| Eng 2 — AI/Assessment                | Assessment Orchestrator (hybrid retrieval, provider selection, retries), embeddings + RagRetriever + quality gate, Mock/Real provider abstractions, structured output validation, AI Observability, AI Ops dashboard |
...
| Days | Focus                                                                                                                                                                                                                                                                                                               |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | Freeze Section 8 shared contracts. Config strategy applied. Skeleton FastAPI + Next.js apps talking to each other over one dummy endpoint.                                                                                                                                                                          |
| 2–3  | Eng 1: state machine + Context / six Derived Facts working headless. Eng 2: MockProvider + hybrid retrieval stub (deterministic path first) + one real provider. Eng 3: floor plan renders, asset click works against fixture data. Eng 4: one YAML scenario drafted, Review/Decision screens against fixture data. |
| 4–5  | Full pipeline connected end to end: Context → Derived Facts → RAG/deterministic retrieval → Assessment → WebSocket → frontend update. Seed embeddings. Second AI provider (Ollama) wired in.                                                                                                                        |
...
| Hybrid retrieval (RAG + deterministic fallback) instead of RAG-only or rules-only | AI hackathon needs a visible retrieval story; PS and judges will ask "show me RAG"; AgriBloom-style quality gate avoids hollow unused RAG | Extra seeding (`knowledge_chunks`), embedding provider, and threshold tuning; deterministic path must stay solid as the cut-last floor |
| pgvector inside Postgres (not Chroma/Pinecone)                                    | Keeps one database / no extra vector service                                                                                              | Depends on Postgres image with pgvector; embedding dim must match at seed time                                                         |
| Next.js App Router UI (not Vite SPA)                                              | Team familiarity; easier UI deploy/hosting                                                                                                | Must keep business APIs on FastAPI — no dual backend; Twin/WS live in Client Components                                               |
...
**Amendment (pre-build):** Sections 5.3, 5.4, 8, 9, and 10 were deliberately expanded before coding began to (1) grow Derived Facts from three problem-statement examples to six rules, and (2) adopt hybrid retrieval (RAG primary + quality-gated deterministic fallback) under Orchestrator control. This supersedes archived ADR-006. The Compound Risk signature sequence (Section 12) and the three canonical facts it depends on are unchanged.